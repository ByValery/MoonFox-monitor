...
function Repair-MojibakeObject($obj) {
  $changed = $false
  if ($null -eq $obj) { return $false }
  if ($obj -is [array]) {
    foreach ($item in @($obj)) {
      if (Repair-MojibakeObject $item) { $changed = $true }
    }
    return $changed
  }
  if ($obj -is [pscustomobject]) {
    foreach ($p in @($obj.PSObject.Properties)) {
      if ($p.Value -is [string]) {
        $fixed = Repair-MojibakeText ([string]$p.Value)
        if ($fixed -ne $p.Value) {
          $obj.($p.Name) = $fixed
          $changed = $true
        }
      } elseif ($p.Value -is [array] -or $p.Value -is [pscustomobject]) {
        if (Repair-MojibakeObject $p.Value) { $changed = $true }
      }
    }
  }
  return $changed
...
function Repair-MojibakeDbFast($db) {
  $changed = $false
  foreach ($part in @($db.settings, $db.sites, $db.routers, $db.events, $db.graphs)) {
    if (Repair-MojibakeObject $part) { $changed = $true }
  }
  return $changed
}
...
function Load-Db {
  ...
  if (Repair-MojibakeDbFast $db) { $needsSave = $true }
...
function Save-Db($db) {
  $json = $db | ConvertTo-Json -Depth 30 -Compress
...
function Get-StatePayload($db) {
  $history = @($db.history)
  $historyTail = @($history | Select-Object -Last 500)
...
function Get-HistoryPayload($db, $body) {
  $kind = [string]$body.kind
  if ($kind -notin @('site','router')) { $kind = 'site' }
  $minutes = 60
  if ($null -ne $body.minutes) { $minutes = [int]$body.minutes }
  if ($minutes -lt 1) { $minutes = 1 }
  if ($minutes -gt 10080) { $minutes = 10080 }
  $cutoff = (Get-Date).AddMinutes(-$minutes)
  $ids = @()
  if ($null -ne $body.objectIds) {
    $ids = @($body.objectIds | ForEach-Object { [string]$_ } | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
  }
  $items = @($db.history | Where-Object {
    $keep = $true
    if ($_.kind -ne $kind) { $keep = $false }
    if ($keep -and $ids.Count -gt 0 -and $_.objectId -and $_.objectId -notin $ids) { $keep = $false }
    if ($keep) {
      try {
        if (-not [string]::IsNullOrWhiteSpace([string]$_.ts)) {
          $keep = ([datetime]::Parse([string]$_.ts)) -ge $cutoff
        }
      } catch {
        $keep = $true
      }
...
function Prune-History($db) {
  ...
  if ([string]::IsNullOrWhiteSpace($mode)) { $mode = 'days' }
  $items = @($db.history)
  if ($mode -eq 'days') {
  ...
  $maxRecords = GetSettingInt $db 'historyMaxRecords' 10000
  if ($maxRecords -lt 50) { $maxRecords = 50 }
  if ($maxRecords -gt 1000000) { $maxRecords = 1000000 }
...
function Ensure-SettingsDefaults($db) {
  ...
  EnsureSetting $db 'historyRetentionMode' 'days'
  EnsureSetting $db 'historyRetentionDays' 7
  EnsureSetting $db 'historyMaxRecords' 10000
...
while ($listener.IsListening) {
  ...
  try {
    ...
    if ($path -like '/api/*') {
      $db = Load-Db
      switch -Regex ($path) {
        '^/api/export/config$' { Send $ctx ($db | ConvertTo-Json -Depth 30); continue }
        '^/api/state$' { Send $ctx ((Get-StatePayload $db) | ConvertTo-Json -Depth 30); continue }
        '^/api/history/query$' {
          $b = ReadBody $ctx | ConvertFrom-Json
          Send $ctx ((Get-HistoryPayload $db $b) | ConvertTo-Json -Depth 12)
          continue
        }
        '^/api/check$' { CheckAll $db; Save-Db $db; Send $ctx ((Get-StatePayload $db) | ConvertTo-Json -Depth 30); continue }
        '^/api/site/add$' {
          $b = ReadBody $ctx | ConvertFrom-Json
